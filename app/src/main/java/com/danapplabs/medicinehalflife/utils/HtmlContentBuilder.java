package com.danapplabs.medicinehalflife.utils;
...
public class HtmlContentBuilder {
    ...
    public String build() {
        ...
        String content = "<html> <head> <script type=\"text/javascript\" src=\"https://www.gstatic.com/charts/loader.js\"></script> <script type=\"text/javascript\">"
                ...
                + "]); var options = { "
                + "title: 'Medicine Half-life Simulation'"
                + ", vAxis: {title: 'Concentration ("
                + doseUnit
                + ")'}, hAxis: {title: 'Time"
                //+ halflifeUnit
                + "', showTextEvery: "
                + ((int)(this.mSimulation.concentrations.length*0.95/4))
                +", slantedText: true}, curveType: 'function', legend: { position: 'bottom' } }; var chart = new google.visualization.LineChart(document.getElementById('curve_chart')); chart.draw(data, options); } </script> </head> <body> <div id=\"curve_chart\" style=\"width: "
                + webview_width
                + "px; height: "
                + webview_height
                + "px\"></div> </body> </html>";
    ...
    private String GenerateDataTable(TakenDrug simulation) {
        ...
        for (int i = 0; i < simulation.concentrations.length; i++) {
            ret += "['" + i+" "+simulation.axisTimeUnit + "'," + simulation.concentrations[i] + "]";
